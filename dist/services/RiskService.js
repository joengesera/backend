...
class RiskService {
    static async analyzeCourse(courseId, userId) {
        ...
        const nextExamWork = course.works.find((w) => String(w.workTypeLabel ?? w.workType?.type ?? '').toUpperCase() === 'EXAMEN' && !!w.dueDate) ?? null;
        const gradeFactor = this.calculateGradeScore(course.grades);
        const workloadFactor = this.calculateWorkloadScore(tasks);
        const urgencyFactor = this.calculateUrgencyScore(nextExamWork?.dueDate ?? nextExamEvent?.startDate ?? null);
    ...
    static calculateGradeScore(grades) {
        if (grades.length === 0)
            return 40; // Score neutre si pas de notes
        const averageOn20 = PointsEngineService_1.PointsEngineService.calculatePercentageBasedAverage(grades);
    ...
    static calculateUrgencyScore(nextExamDate) {
        if (!nextExamDate)
            return 20;
        const now = new Date();
